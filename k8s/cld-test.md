...
1. 의존성 맵핑 문서화
2. Upstream 팀 서비스 장애 시 Downstream 팀의 graceful degradation 검증
3. Timeout 및 retry 설정 적정성 확인
4. Circuit breaker 패턴 적용 여부 점검
...
2. **Cilium 주의**: eBPF 고성능 모드에서 네트워크 카오스 테스트 시 `bpf.hostLegacyRouting=true` 필요
3. **Dev 우선**: 80% 시나리오를 Dev에서 검증 후 Production 확대
4. **BGP 테스트**: Production에서만 가능한 BGP failover, ECMP 분산은 별도 윈도우 확보
5. **자동 중단**: Prometheus 프로브로 error rate > 5% 시 즉시 실험 중단
6. **팀 격리**: Namespace별 RBAC, opt-in 라벨로 blast radius 제한
7. **문서화**: 모든 SPOF 발견 시 즉시 문서화 및 우선순위 매트릭스로 분류
8. **점진적 확대**: Phase 1(Critical) → Phase 2(Important) → Phase 3(Nice-to-have) 순서 준수
